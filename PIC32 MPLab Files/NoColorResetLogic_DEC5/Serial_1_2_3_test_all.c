...
static PT_THREAD (protothread_param(struct pt *pt))
{
	...
	while (1) {
        ...
        switch (state) {
            case 1: // continue moving right; start drawing pixels
            ...
            case 2: // continue moving left; start drawing pixels
            ...
            case 4:
                ...
                while (!reset)
                {
                    //Clear Corresponding Section of the TFT Display
                    tft_setCursor(30, 280);
                    sprintf(buffer, "Dark Blue - No Fruit");
                    tft_writeString(buffer);
                    tft_setCursor(30, 290);
                    sprintf(buffer, "Dark Purple - Fruit Detected - No Ripeness Detected");
                    tft_writeString(buffer);
                    tft_setCursor(30, 300);
                    sprintf(buffer, "Red - Fruit Detected - Ripeness Detected");
                    tft_writeString(buffer);
                    tft_setCursor(30, 310);
                ...
                tft_setTextColor(ILI9340_YELLOW); tft_setTextSize(1.5);
                tft_setCursor(30, 280);
                sprintf(buffer, "Please wait for the sensor to move back to the startup position.");
                tft_writeString(buffer);
                ...
                for (q = 0; q < 20; q++)
                {
                    //Move up until sensor is at reset position
                    mPORTBClearBits(BIT_3 | BIT_4 | BIT_7);
                    mPORTBSetBits(BIT_7);
                    PT_YIELD_UNTIL(pt, mPORTBReadBits(BIT_8) == 256);
                }
