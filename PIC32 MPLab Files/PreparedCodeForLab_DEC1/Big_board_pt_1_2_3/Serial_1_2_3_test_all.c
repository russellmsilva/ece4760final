...
//TODO: User Interface Thread to enter fruit type - button and potentiometer
//Drawing Color Map on the TFT
//Logic to determine if user-specified fruit is ripe or not (2d array or ring buffer?)
//Ability to restart program with button push
static PT_THREAD (protothread_param(struct pt *pt))
{
	...
	while (1) {
        ...
        tft_setCursor(30, 160);
        sprintf(buffer, "610nm Value: %d", delimited[0]);
        tft_writeString(buffer);
        tft_setCursor(30, 170);
        sprintf(buffer, "680nm Value: %d", delimited[1]);
        tft_writeString(buffer);
        
        switch (state) {
            case 0:  // init / reset
        
                if (*delimited[0] < 2900 && *delimited[0] > 2650){
                    state = 2; //go blue
                }

                else if (*delimited[0] < 1030 && *delimited[0] > 850){
                    state = 3;
                }
            ...
            case 2: // blue             
                ...
                if (*delimited[0] < 1030 && *delimited[0] > 850){
                    state = 3;
                }
            ...
            case 4: // continue moving right; start drawing pixels
                ...
                ripe_buffer[(sensor_pos + 1)%8] = *delimited[1];
            ...
            case 6: // move down
                ...
                else if (total_steps_down = max_down){
                    state = 7;
                }        
                break;
